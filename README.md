...
| Operators & Built-in Functions | `jzero-codegen`, `jzero-vm` | Ch. 15 | ✅ Done |
...
| `stringpool.java` | `StringPool` in `jzero-vm` | Runtime string interning for concatenation |
...
- **String concatenation** — `String + String` is allowed by the type checker and emits `SADD` in the TAC layer. `String - String` is rejected.
